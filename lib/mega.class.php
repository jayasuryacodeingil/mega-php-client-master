...
class MEGA
{
  ...
  public const SERVER_GLOBAL = 'g.api.mega.co.nz';

  /* Europe MEGA API server endpoint. */
  public const SERVER_EUROPE = 'eu.api.mega.co.nz';

  /* Enable debug message output. */
  public const DEBUG = true;

  /* Default server endpoint. */
  private static string $server = self::SERVER_GLOBAL;
  ...
  /* User session ID */
  private ?string $u_sid = null;

  /* User master key */
  private ?string $u_k = null;

  /* User master key as string */
  private ?string $u_k_aes = null;

  /* User RSA private key */
  private ?string $u_privk = null;
  ...
  {
    // Build AES key using the same xor pattern as the original implementation
    ...
    foreach ($chunks as $chunk_start => $chunk_size) {
      ...
      // Decrypt encrypted chunk using AES-CTR via OpenSSL. We compute the
      // counter position for this chunk so the stream decryption matches the
      // original mcrypt CTR behavior.
      $block_index = intdiv($chunk_start, 16);
      $chunk = $this->aes_ctr_decrypt($chunk, $aeskey, $iv, $block_index);
  ...
  /**
   * Decrypt a ciphertext chunk using AES-128-CTR with OpenSSL.
   * $block_index is the number of 16-byte blocks to advance the initial IV.
   */
  protected function aes_ctr_decrypt($ciphertext, $aeskey, $iv, $block_index)
  {
    // Interpret IV as four big-endian 32-bit words
    $iv_words = array_values(unpack('N4', $iv));

    // Add block_index to the 128-bit big-endian counter represented by $iv_words
    $carry = $block_index;
    for ($i = 3; $i >= 0; $i--) {
      $sum = ($iv_words[$i] & 0xFFFFFFFF) + ($carry & 0xFFFFFFFF);
      $iv_words[$i] = $sum & 0xFFFFFFFF;
      $carry = ($sum >> 32) & 0xFFFFFFFF;
    }
    $iv_block = pack('N4', $iv_words[0], $iv_words[1], $iv_words[2], $iv_words[3]);

    $plaintext = openssl_decrypt($ciphertext, 'aes-128-ctr', $aeskey, OPENSSL_RAW_DATA, $iv_block);
    if ($plaintext === false) {
...
class MEGACrypto
{
  ...
  {
    // Use OpenSSL AES-128-CBC with zero IV and zero padding to emulate
    // mcrypt's behavior with manual zero-padding in this library.
    $iv = str_repeat("\0", 16);
    $enc = openssl_encrypt($data, 'aes-128-cbc', $key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv);
    if ($enc === false) {
      throw new RuntimeException('AES-CBC encryption failed');
    }
    return $enc;
  }
  ...
  {
    $iv = str_repeat("\0", 16);
    $dec = openssl_decrypt($data, 'aes-128-cbc', $key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv);
    if ($dec === false) {
      throw new RuntimeException('AES-CBC decryption failed');
    }
    return $dec;
  }
...
class MEGARsa
{
  ...
  private static function bin2int($str)
  {
    ...
    $n = strlen($str);
    do {
      $result = bcadd(bcmul($result, '256'), strval(ord($str[--$n])));
    } while ($n > 0);
    return $result;
  }
  ...
  private static function int2bin($num)
  {
    $result = '';
    do {
      $result .= chr(intval(bcmod($num, '256')));
      $num = bcdiv($num, '256');
    } while (bccomp($num, 0));
    return $result;
  }
  ...
  private static function bitOr($num1, $num2, $start_pos)
  {
    ...
    $num2 = bcmul($num2, strval(1 << $start_bit));
...
class MEGAUtil
{
  ...
  public static function mpi2b($s)
  {
    ...
    $n = 0;
    for ($i = 0; $i < $len; $i++) {
      $n = bcadd($n, bcmul(strval(hexdec($s[$i])), bcpow('16', (string)($len - $i - 1))));
    }
    return $n;
  }
